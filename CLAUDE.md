...
**Mermaid rendering:** Markdig's `UseAdvancedExtensions()` emits fenced Mermaid blocks as `<pre class="mermaid">`. JS hides the `<pre>`, injects a `<div class="mermaid-rendered">` alongside it, and renders each diagram individually via `mermaid.render()` (`appInterop._renderMermaidInto` in `wwwroot/js/app.js`). On document change, `processContent` removes those injected divs and resets `data-mermaid-processed` before re-running.

**xyChart label rotation:** `_renderMermaidInto` renders each diagram once, then calls `_xAxisLabelsOverlap` to check whether an xyChart-beta (bar/line chart) diagram's x-axis category labels collide (mermaid itself does no collision detection there — mermaid-js/mermaid#5926). If they do, it re-renders that one diagram with a `%%{init: {"xyChart": {"xAxis": {"labelRotation": 90}}}}%%` directive prepended to rotate the labels fully vertical (90°) — kept binary with horizontal rather than an in-between angle like 45° so a label's association with its bar stays unambiguous even when label lengths vary a lot, a config option added in mermaid v11.16 (the vendored `wwwroot/js/mermaid.min.js`). `Platforms/Windows/MermaidRenderer.cs`'s export path duplicates this same detect-then-rotate logic inline (it drives mermaid.js in a hidden WebView2, so it can't call back into `app.js`) — keep both in sync if this changes. The overlap check uses `getBoundingClientRect()`, not `getBBox()`: xyChart positions each label via a `transform="translate(...)"` on the `<text>` itself with `x="0" y="0"`, so `getBBox()` returns nearly the same local-origin box for every label regardless of actual position.
